...
- Public data gate runner writes `public_data_gate.md`, so recording status, alpha, fills, churn, spread capture, adverse selection, risk blocks, quality replays, diagnostic replays, split sanity replays, and output paths are reviewable without parsing JSON.
...
- Last checked: 2026-05-15 07:18 JST, duration 10.48h, strict recording status `ok_for_replay=true`, stale `false`, sequence violations 0, max event gap 5.973s, duration gate `10.48<24h`, 3 rotated files totaling about 286.5 MB.
...
- Completion watcher was replaced again at 2026-05-14 23:31 JST without stopping the recorder. Active watcher PID is 28024; old watcher PID 20144 was stopped after the new watcher started.
...
- The active watcher writes its stdout/stderr to `data/logs/bitbank_btc_jpy_25h_retry_20260514_204916_gate_after_v5.log` and status to `data/logs/bitbank_btc_jpy_25h_retry_20260514_204916_gate_after_v5.status.txt`.
