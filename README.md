...
The CLI expands `--events` wildcards itself, so the `*.jsonl` form works in PowerShell. On Windows, `--prevent-sleep` keeps the system awake during the post-recording replay and quality-gate pass.
